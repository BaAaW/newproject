DateInput = (function($) {
	...
	DateInput.prototype = {
		...
		hideIfClickOutside: function(event) {
			var o1 = event.target;
			var o2 = this.input[0];
			var o3 = this.insideSelector(event);
			if (event.target != this.input[0] && !this.insideSelector(event)) {
				this.hide()
			}
		},
		insideSelector: function(event) {
			return event.target.className == 'button prev' || event.target.className == 'button next' || event.target.className == 'date_selector';
