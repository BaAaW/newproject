...
"ge":"盖哥歌搁戈鸽胳疙割革葛格蛤阁隔铬个各咯鬲仡哿圪塥嗝纥搿膈铪镉袼虼舸骼",
"gei":"给",
"gen":"根跟亘茛哏艮",
"geng":"耕更庚羹埂耿梗颈哽赓绠鲠",
...
"guang":"光广逛咣犷桄胱",
"gui":"瑰规圭硅归龟闺轨鬼诡癸桂柜跪贵刽炔匦刿庋宄妫桧炅晷皈簋鲑鳜姽",
"gun":"辊滚棍衮绲磙鲧",
...
"hua":"花哗华猾滑画划化话骅桦砉铧婳",
"huai":"槐徊怀淮坏踝",
"huan":"欢环桓还缓换患唤痪豢焕涣宦幻郇奂垸萑擐圜獾洹浣漶寰逭缳锾鲩鬟阛",
"huang":"荒慌黄磺蝗簧皇凰惶煌晃幌恍谎隍徨湟潢遑璜肓癀蟥篁鳇",
"hui":"灰挥辉徽恢蛔回毁悔慧卉惠晦贿秽会烩汇讳诲绘溃诙茴荟蕙咴哕喙隳洄浍彗缋桧晖恚虺蟪麾阓",
"hun":"荤昏婚魂浑混诨馄阍溷珲",
...
"jiao":"蕉椒礁焦胶交郊浇骄娇嚼搅铰矫侥脚狡角饺缴绞剿教酵轿较叫窖觉校佼僬艽茭挢噍峤徼姣敫皎鹪蛟醮跤鲛嘦嘂",
...
"tao":"掏涛滔绦萄桃逃淘陶讨套鼗叨啕洮韬焘饕弢",
"te":"特忒忑铽",
"teng":"藤腾疼誊滕",
"ti":"梯剔踢锑提题蹄啼体替嚏惕涕剃屉倜荑悌逖绨缇鹈裼醍",
"tian":"天添填田甜恬舔腆掭忝阗殄畋钿锘兲",
...
"xie":"解楔些歇蝎鞋协挟携邪斜胁谐写械卸蟹懈泄泻谢屑血偕亵勰燮薤撷獬廨渫瀣邂绁缬榭榍颉躞鲑骱",
...
"yao":"侥啮疟邀腰妖瑶摇尧遥窑谣姚咬舀药要耀钥夭爻吆崤崾徭幺珧杳轺曜肴铫鹞窈繇鳐垚",
...
"yin":"茵荫因殷音阴姻吟银淫寅饮尹引隐印胤鄞廴垠堙茚吲喑狺夤洇湮氤铟瘾窨蚓霪龈烎",
...
/*通过查找字典得到与uncode码对应的拼音*/
function _getPinyin(code)
{
   var _pinyin="N/A";
   var findIt=false;

   $.each(_pinyin_uncode_map, function(key,value){
...
function toPinyin(objq){
	if($("#insuredRadio").attr("checked")){
		if(objq.id=="Applicant_fullName0"||objq.id=="insuredRadio"){
			 var uniString=$("#Applicant_fullName0").val();
			   var pinyins = "";
			   $.each(uniString,function(i){
				   if(uniString.charCodeAt(i)>122){
					   pinyins+=_getPinyin(uniString.charCodeAt(i))+" ";
				   }else{
					   pinyins+=uniString.charAt(i);
				   }
			   });
				$("#Insured_insuredEnName0").attr("value",pinyins.toUpperCase());
		}else{
			$("#Insured_insuredEnName0").attr("value","");
		}
	}else	if($("#otherInsuredRadio").attr("checked")){
		if(objq.id=="Insured_fullName0"||objq.id=="otherInsuredRadio"){
			 var uniString=$("#Insured_fullName0").val();
			   var pinyins = "";
			   $.each(uniString,function(i){
				   if(uniString.charCodeAt(i)>122){
					   pinyins+=_getPinyin(uniString.charCodeAt(i))+" ";
...
function changeToPinyin(cvalue){
	//·,
	 cvalue = cvalue.replace(/[·,]+/g,'');	
	   var pinyins = "";
	   $.each(cvalue,function(i){
		   if(cvalue.charCodeAt(i)>122){
			   pinyins+=_getPinyin(cvalue.charCodeAt(i))+" ";
			   pinyins = pinyins.toUpperCase();
		   }else{
			   pinyins+=cvalue.charAt(i);
			   pinyins = pinyins.toUpperCase();
		   }
	   });
	   return pinyins;	   
...
function changeToPinyin_new(obj,cvalue){
	if (cvalue == ""){
		return cvalue;
	}
	var reStr = "";
	$.ajax({
		url : '/mall/sale/shortterm/getPingYin',
		type : "post",
		data : "cvalue="+cvalue,
		dataType : "json",
...
function getValue(){
	...
	}else if(reStr == oldStr ){
		reStr = changeToPinyin_old(oldStr);
		//return reStr;
	}//else{
	//	return reStr;
	//}
