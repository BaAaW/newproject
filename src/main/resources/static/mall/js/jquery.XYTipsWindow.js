...
;(function(){
	$.XYTipsWindow=function(o){
		defaults = $.extend({
			...
			___closeID:"",
			___triggerID:"",
			___boxBdColor:"#cacaca",
			___boxBdOpacity:"1",
			___boxWrapBdColor:"#2E68CF",
			___windowBgColor:"#000000",
			___windowBgOpacity:"0.3",
			___time:"",
			___drag:"",
			___dragBoxOpacity:"1",
			___showTitle:true,
	...
	$.extend($.XYTipsWindow,{
		...
		init: function (o){
			...
			if(o.___showbg != "" && o.___showbg == true){
				var $boxBgDom = "<iframe id=\"XYTipsWindowBg"+o.___boxID+"\" class=\"XYTipsWindowBg\" style=\"position:absolute;height:"+$(document).height()+"px;filter:alpha(opacity=0);opacity:0;width:100%;left:0;top:0;z-index:870618;\"></iframe>";
				$($boxBgDom).appendTo("body").animate({opacity:o.___windowBgOpacity},0).end();
//				$("#XYTipsWindowBg").dblclick(function(){
//					$.XYTipsWindow.removeBox();
//				});
			};
			if(o.___drag != "") {
		...
		showBox: function(o) {
			...
			var $boxDom = "<div id=\""+o.___boxID+"\" class=\"XYTipsWindow\">";
				...
				$boxDom += "</div></div>";
				$boxDom += "<iframe src=\"about:blank\"  scrolling=\"no\" style=\"position:absolute;left:0;top:0;filter:alpha(opacity=0);opacity:0;height:0;scrolling=no;z-index:10714\"></iframe>";
				$boxDom += "</div>";
		...
		setBoxzIndex: function (o) {
			...
			$box.die().live("click",function(){
				for(var i=0; i < $XYTipsWindowarr.length; i++){
					$XYTipsWindowarr[i].style.zIndex = 891208;
				};
		...
		//关闭弹出层
		removeBox: function (){
			if(BOXID.___close != "" && $.isFunction(BOXID.___close)){
				BOXID.___close.call(this);
			}
			...
			if($box != null || $boxbg != null){
				...
				if ($contentType == "id") {
					...
					$boxbg.animate({opacity:"0"},500,function(){$(this).remove().hide();});
		...
		//关闭指定弹出层
		removeBoxNew: function (o){
			if(o.___close != "" && $.isFunction(o.___close)){
				o.___close.call(this);
			}
			var $box = $("#"+o.___boxID);
			var $boxbg = $("#XYTipsWindowBg"+o.___boxID);
			if($box != null || $boxbg != null){
				var $contentID = $(".___boxContent",$box);
					$contentType = o.___content.substring(0,o.___content.indexOf(":"));
					$content = o.___content.substring(o.___content.indexOf(":")+1,o.___content.length);
